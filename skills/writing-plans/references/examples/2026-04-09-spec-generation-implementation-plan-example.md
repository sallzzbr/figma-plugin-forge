...
**Date**: 2026-04-09
**Design doc**: [`2026-04-09-spec-generation-design-doc-example.md`](2026-04-09-spec-generation-design-doc-example.md)
...
`enrichSpecs(request, { endpoint, getAuthToken })` returns a typed `{ ok: true; data } | { ok: false; error: EnrichError }` result (see the [optional-backend snippet](../../../figma-backend-integration/references/optional-backend-snippet.md)). The UI handles the enrich button: disables it while in flight, replaces local state on success, renders the typed error on failure with a retry button. The raw local spec remains copyable throughout.
